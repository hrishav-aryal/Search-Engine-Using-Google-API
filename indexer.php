...
$password = "";
    ...
    $seen = array();
    $pagequery = "SELECT url, last_index from Page";
    $result = $mysqli->query($pagequery);
    if($result){
        while($row = $result->fetch_assoc()){
            $seen[$row['url']] = $row['last_index'];
        }
    }
    $_SESSION['seen']=$seen;
    if(isset($seen[$url]) && (strtotime(date('Y-m-d H:i:s'))-strtotime($seen[$url])<604800)){
      ...
      if($title==false){
        ...
        if(!isset($seen[$url])){
            ...
            $newpagequery = "INSERT INTO Page(url, title, description, last_index) VALUES ('$url', '$title','$description', '$date')";
...
function getPagesToCrawl($url, $mysqli, $dom = null){
  ...
  $count = 0;
  $toCrawl = json_decode($_SESSION['pagesToCrawl'], true);
  ...
  if($count!=0){
      $_SESSION['pagesToCrawl'] = json_encode($toCrawl);
  }
}
function getNextUrl($url, $mysqli){
  $arr = json_decode($_SESSION['pagesToCrawl'], true);
  if(empty($arr)){
